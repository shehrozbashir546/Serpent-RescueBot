...
void whitelist(int safe){
    printf("\nLocation whitelisted!: %d\n", safe);
    goodlist[goodcounter] = safe; //mark location that robot walks on
    goodcounter++;
}

int allowed(int goodLocation){
    for (int i = 0; i<200; i++){
        if (goodLocation == goodlist[i]) {return true;} //location blacklisted
    }
    return false;
}
...
int forbidden(int forbiddenLocation){
    ...
    for (int i = 0; i<200; i++){
        if (forbiddenLocation == forbiddenlist[i]) {            
         /*  if (allowed(forbiddenLocation)){
                goodlist[forbiddenLocation]=0;
          } */ 
            return true;  
        } //location blacklisted
       //remove from whitelist if its there
    }
    return false;
}
...
int move(char *world, int map_id) {
    ...
    //add robot index to whitelist every move.

    if (target_index>0) {whitelist(robot_index);}
    //print whitelist: 
    //for (int j=0;j<200;j++){printf("white list: %d", goodlist[j]);}
 ...
 //reset forbidden list    
        ...
        if (world[rup] == '#' || forbidden(rup)) { 
            ...
            if (countleft > countright ) {
                ...
                for (int i=0; i<moveright;i++){
                    if(driveMode(world[rup])) return drivemode;
                    //check if above the star an obstacle, then there is no point to destroy it
                    if (destructable(world[rup])) {
                        if (world[rup-21] != '#' || forbidden(rup-21) == false) return up_destroy;
                        else {blacklist(rup); goto checkpoint;}
                    }
    ...
    if((hR-hT) < 0 && NoHorizontal == false) {
        ...
        if(world[rright]=='#' || forbidden(rright)){
            ...
            //printf("\n countup: %d, countdown:%d", countup, countdown);
