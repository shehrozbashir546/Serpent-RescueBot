...
int move(char *world, int map_id) {
    ...
    if((vR-vT) > 0 && NoVertical == false){ 
        ...
        if(driveMode(world[rup])) return drivemode;
        if (destructable(world[rup])&&(world[(rup-21)] != '#')) { printf("destructable! %c", world[rup-21]);
            if ( || (forbidden(rup-21) == false)) { return up_destroy;}
            else if ((world[(rup-21)] == '#')) {blacklist(rup); goto checkpoint;}
        }
        return up; 
        NoHorizontal=0;
    }   
