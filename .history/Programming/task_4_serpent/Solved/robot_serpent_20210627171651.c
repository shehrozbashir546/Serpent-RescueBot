...
int up_destructable(char infront) {
    if (destructable(infront)&&((infront-21) != '#')) return up_destroy;
...
int right_destructable(char infront) {
    if (destructable(infront)&&((infront+1) != '#')) return right_destroy;
...
int down_destructable(char infront) {
    if (destructable(infront)&&((infront+21) != '#')) return down_destroy;
    else if ((destructable(infront) && (infront+21 == '#'))){
        blacklist(infront); 
        return true;
    }
...
int left_destructable(char infront) {
    if (destructable(infront)&&((infront-1) != '#')) return left_destroy;
    else if ((destructable(infront) && (infront-1 == '#'))){
        blacklist(infront); 
        return true;
    }
...
int move(char *world, int map_id) {
    ...
    if((vR-vT) > 0 && NoVertical == false){ 
        ...
        if (world[rup] == '#' || forbidden(rup)) { 
            ...
            if (countleft < countright || countleft == countright) {
                if (((world[rleft]) == '#' || forbidden(rleft))){
                    ...
                    right_destructable(world[rright]);
                    if (right_destructable(world[rright])) goto checkpoint;
                    return right;  
                }  
                ...
                do{                 
                    ...
                    left_destructable(world[rleft]);
                    if (left_destructable(world[rleft])) goto checkpoint;
                    return left;
                }
                ...
                for (int i=0; i<moveleft;i++){
                    ...
                    up_destructable(world[rup]);
                    if (up_destructable(world[rup])) goto checkpoint;
                    return up;
                }
            }
            ...
            if (countleft > countright ) {
                if (((world[rright]) == '#' || forbidden(rright))){
                    ...
                    if (driveMode(world[rleft])) return drivemode;
                    //check if obstacle must be destroyed, destroy if yes
                    if (left_destructable(world[rleft])) return left_destroy;
                    else if (left_destructable(world[rleft]) == false) goto checkpoint;
                    return left;  
                }  
                //go right until up is clear
                do{ 
                    if (driveMode(world[rright])) return drivemode;
                    if (up_destructable(world[rright])) return right_destroy;
                    else if (right_destructable(world[rright]) == false) goto checkpoint;
                    return right;
                ...
                for (int i=0; i<moveright;i++){
                    if(driveMode(world[rup])) return drivemode;
                    //is it worth destroying obstacle? destroy if yes
                    if (up_destructable(world[rup])) return up_destroy;
                    else if (up_destructable(world[rup]) == false) goto checkpoint;
                    return up;
                }
                }  
        }
        ...
        if(driveMode(world[rup])) return drivemode;
        if (up_destructable(world[rup])) return up_destroy;
        if (up_destructable(world[rup]) == false) printf("NOT WORTH DESTROYING");
        return up; 
        NoHorizontal=0;
    }   
    ...
    if ((vR-vT) < 0 && NoVertical == false){  
        ...
        if (world[rdown] == '#' || forbidden(rdown)) {
            ...
            if (countleft < countright || countleft == countright) {    
                if (((world[rleft]) == '#' || forbidden(rleft))){
                    blacklist(robot_index); 
                    if (driveMode(world[rright])) return drivemode;
                    if (right_destructable(world[rright])) return right_destroy;
                    else if (right_destructable(world[rright]) == false) goto checkpoint;
                    return right;  
                }  
                do{                     
                ...
                for (int i=0; i<moveleft;i++){
                    ...
                    if (down_destructable(world[rdown])) return down_destroy;
                    else if (down_destructable(world[rdown]) == false) goto checkpoint;
                    return down;
                }
            }
        ...
        if(driveMode(world[rdown])) return drivemode;
        if (down_destructable(world[rdown])) return down_destroy;
        if (down_destructable(world[rdown]) == false) printf("BUGGED");
        return down;   
        NoHorizontal=0;
    }
    ...
    if((hR-hT) < 0 && NoHorizontal == false) {
        ...
        if (driveMode(world[rright])) return drivemode;
        if (right_destructable(world[rright])) return right_destroy;
        if (right_destructable(world[rright]) == false) printf("NOT OWRHT");
        return right; 
        NoVertical=0;
    }
    ...
    //LEFT
        ...
        //check if obstacle must be destroyed, destroy if yes
        if (left_destructable(world[rleft])) return left_destroy;
        if (left_destructable(world[rleft]) == false) printf("FUCK");
        return left;
        NoVertical=0;
    }
